 ...
 */
@PageObject(css = ConfigDialogAdobeClassicImpl.FORM_CQ_DIALOG)
public class ConfigDialogAdobeClassicImpl implements ConfigDialogAdobeClassic {
	...
	@FindBy(css = ".coral-TabPanel-tab.is-active")
	...
	public void verifyFullscreen() {
		bobcatWait.until(
				webDriver -> containsIgnoreCase(dialog.getAttribute(HtmlTags.Attributes.CLASS), FULLSCREEN_CLASS));
	}
	 ...
	 */
	@Override
	public ConfigDialogAdobeClassic setField(String label, String fieldType, Object value) {
	 ...
	 */
	@Override
	public ConfigDialogAdobeClassic switchTab(String tabLabel) {
		if (!tabs.isEmpty()) {
			tabs.stream() //
	...
	private void setFields(List<FieldConfig> value) {
		...
		value.forEach(fieldConfig -> {
			DialogField field = dialogConfigurer.getDialogField(parent, fieldConfig.getLabel(), fieldConfig.getType());
